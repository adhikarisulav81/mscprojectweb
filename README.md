...
They can view the submitted request.
They can edit and update their request if not assigned.
They can set the priority of their request either high or normal, depending on the urgency.
They can track their request. They can view their request status, either assigned to a technician, not assigned, work started, rejected, or completed.
They receive the notification of technician assignment and request completion.
...
They click on the assign button, and the system automatically assigns the request to the best-fit technician based on the active workload.
They can add the device model and service type, along with the price that they offer.
They can view all the requests. They can filter the request based on status (assigned, not assigned, work started, rejected, and completed) and priority (high and normal).
They can view the invoice for completed requests.
